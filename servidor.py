...
MANAGER_PIN  = os.environ.get("MANAGER_PIN", "")  # PIN do gerente para área admin no PWA
...
def _cors(response):
    ...
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, X-Token, X-Admin-Key, X-Manager-Pin"
...
def listar_funcionarios_resumo():
    conn = _conn()
    cur  = _run(conn, """
...
# ──────────────────────────────────────────────────────────────────────────────
# ROTAS DO GERENTE — área admin do PWA
# ──────────────────────────────────────────────────────────────────────────────

def _check_manager_pin():
    """Verifica o PIN do gerente enviado no header X-Manager-Pin."""
    if not MANAGER_PIN:
        return False  # sem PIN configurado, bloqueia tudo
    return request.headers.get("X-Manager-Pin", "") == MANAGER_PIN


def _processar_csv(conteudo):
    """Processa CSV com colunas: Funcionario,Data,Horario"""
    import csv, io
    batidas = {}
    reader = csv.DictReader(io.StringIO(conteudo))
    for row in reader:
        nome = (row.get("Funcionario") or row.get("funcionario") or "").strip()
        data = (row.get("Data") or row.get("data") or "").strip()
        hora = (row.get("Horario") or row.get("horario") or "").strip()
        if not nome or not data or not hora:
            continue
        chave = (nome, data)
        if chave not in batidas:
            batidas[chave] = []
        batidas[chave].append(hora)
    return [{"nome": k[0], "data": k[1], "horarios": sorted(v)} for k, v in batidas.items()]


def _processar_txt_rep(conteudo, mapa_pis):
    """Processa TXT formato REP: PIS/Matricula Data Hora"""
    batidas = {}
    for linha in conteudo.splitlines():
        ...
        data = partes[1]
        hora = partes[2]
        if "/" not in data or ":" not in hora:
            continue
        nome = mapa_pis.get(pis, pis)
        chave = (nome, data)
        if chave not in batidas:
            batidas[chave] = []
        batidas[chave].append(hora)
...
def _calcular_saldo_dia(horarios, jornada_min):
    """Calcula saldo do dia em minutos."""
    def hm(t):
        try:
            h, m = t.split(":")
            return int(h) * 60 + int(m)
    ...
    qtd = len(horarios)
    if qtd == 2:
        trabalhado = max(0, hm(horarios[1]) - hm(horarios[0]))
    elif qtd >= 4:
        trabalhado  = max(0, hm(horarios[1]) - hm(horarios[0]))
        trabalhado += max(0, hm(horarios[3]) - hm(horarios[2]))
    elif qtd > 0:
        for i in range(0, qtd - 1, 2):
            trabalhado += max(0, hm(horarios[i+1]) - hm(horarios[i]))
    return trabalhado - jornada_min


@app.route("/api/gerente/login", methods=["POST"])
def api_gerente_login():
    """Valida o PIN do gerente."""
    if not MANAGER_PIN:
        return jsonify({"erro": "MANAGER_PIN nao configurado no servidor."}), 503
    dados = request.get_json(silent=True) or {}
    pin   = str(dados.get("pin", "")).strip()
    if pin == MANAGER_PIN:
        return jsonify({"ok": True}), 200
...
@app.route("/api/gerente/funcionarios")
def api_gerente_funcionarios():
    """Lista todos os funcionarios com saldo (protegido pelo MANAGER_PIN)."""
    if not _check_manager_pin():
    ...
    cur = _run(conn, """
        ...
        LEFT JOIN banco_horas b ON b.funcionario_id = f.id
        WHERE f.ativo = 1
        ORDER BY COALESCE(b.saldo_min, 0) DESC
    """)
    rows = [_d(r) for r in cur.fetchall()]
    conn.close()
    resultado = []
    for r in rows:
        ...
        resultado.append({
            ...
            "nome":          r["nome"],
            "saldo_min":     sm,
            "saldo_fmt":     _saldo_fmt(sm),
            "saldo_horas":   abs(sm) // 60,
            "saldo_mins":    abs(sm) % 60,
            "positivo":      sm >= 0,
            "atualizado_em": (r.get("atualizado_em") or "")[:16],
        })
    return jsonify(resultado), 200


@app.route("/api/gerente/importacoes")
def api_gerente_importacoes():
    """Retorna historico das ultimas 50 importacoes."""
    if not _check_manager_pin():
        return jsonify({"erro": "Nao autorizado."}), 401
    conn = _conn()
    rows = [_d(r) for r in _run(conn, """
        SELECT id, arquivo, status, importado_em, total_reg, total_erros, avisos
        FROM importacoes
...
@app.route("/api/gerente/importar", methods=["POST"])
def api_gerente_importar():
    """
    Recebe arquivo do relogio de ponto (CSV ou TXT), processa e atualiza o banco.
    Header obrigatorio: X-Manager-Pin
    ...
    try:
        ...
        try:
            ...
            return jsonify({"erro": "Nao foi possivel ler o arquivo."}), 400

    ext = nome_arquivo.rsplit(".", 1)[-1].lower() if "." in nome_arquivo else "csv"

    try:
        if ext == "csv":
            registros = _processar_csv(conteudo)
        elif ext in ("txt", "rep"):
            # Mapa PIS vazio — usa PIS como nome (adequado para CSV com nome direto)
            registros = _processar_txt_rep(conteudo, {})
        else:
            return jsonify({"erro": f"Formato '{ext}' nao suportado. Use csv ou txt."}), 400
    except Exception as e:
        return jsonify({"erro": f"Erro ao processar arquivo: {e}"}), 400

    if not registros:
        return jsonify({"erro": "Nenhum registro encontrado no arquivo."}), 400
    ...
    try:
        cur_imp = _run(conn,
            "INSERT INTO importacoes(arquivo, status, importado_em, total_reg, total_erros)"
            " VALUES(?,?,?,?,?)",
            (nome_arquivo, "processando", now, len(registros), 0))
        imp_id = cur_imp.lastrowid
        ...
        for reg in registros:
            ...
            cur_f = _run(conn, "SELECT id, jornada_h FROM funcionarios WHERE nome=? AND ativo=1", (nome,))
            row_f = _d(cur_f.fetchone())

            if not row_f:
                _run(conn,
                    "INSERT INTO funcionarios(nome, criado_em, ativo) VALUES(?,?,1)",
                    (nome, now))
                conn.commit()
                cur_f2 = _run(conn, "SELECT id, jornada_h FROM funcionarios WHERE nome=?", (nome,))
                row_f  = _d(cur_f2.fetchone())
                avisos.append(f"Funcionario '{nome}' criado automaticamente.")
            ...
            fid         = row_f["id"]
            jornada_h   = row_f.get("jornada_h") or 8
            jornada_min = int(float(jornada_h) * 60)
            saldo_dia   = _calcular_saldo_dia(hors, jornada_min)

            cur_b = _run(conn,
                "SELECT id FROM batidas WHERE funcionario_id=? AND data=?",
                (fid, data))
            existe = _d(cur_b.fetchone())

            if existe:
                _run(conn,
                    "UPDATE batidas SET horarios=?, qtd_batidas=?, tem_erro=0, erros=NULL, importacao_id=?"
                    " WHERE funcionario_id=? AND data=?",
                    (",".join(hors), len(hors), imp_id, fid, data))
            else:
                _run(conn,
                    "INSERT INTO batidas(importacao_id, funcionario_id, funcionario_raw, data, horarios, qtd_batidas, tem_erro)"
                    " VALUES(?,?,?,?,?,?,0)",
                    (imp_id, fid, nome, data, ",".join(hors), len(hors)))

            if nome not in saldos_func:
                saldos_func[nome] = {"id": fid, "delta": 0}
            saldos_func[nome]["delta"] += saldo_dia
            total_ok += 1

        for nome, info in saldos_func.items():
            fid = info["id"]

            cur_all = _run(conn, """
                SELECT horarios FROM batidas
                WHERE funcionario_id=? AND tem_erro=0
            """, (fid,))
            todas_batidas = [_d(r) for r in cur_all.fetchall()]

            cur_fj = _run(conn, "SELECT jornada_h FROM funcionarios WHERE id=?", (fid,))
            row_fj = _d(cur_fj.fetchone())
            jornada_h   = (row_fj.get("jornada_h") or 8) if row_fj else 8
            jornada_min = int(float(jornada_h) * 60)

            saldo_total = 0
            for b in todas_batidas:
                hors_b = [h.strip() for h in (b.get("horarios") or "").split(",") if h.strip()]
                saldo_total += _calcular_saldo_dia(hors_b, jornada_min)
            ...
            if abs(info["delta"]) > 0:
                _run(conn,
                    "INSERT INTO banco_horas_mov(funcionario_id, tipo, minutos, descricao, referencia, criado_em)"
                    " VALUES(?,?,?,?,?,?)",
                    (fid,
                     "credito" if info["delta"] >= 0 else "debito",
                     abs(info["delta"]),
                     f"Importacao: {nome_arquivo}",
                     now[:10],
                     now))

        avisos_str = "; ".join(avisos) if avisos else None
        _run(conn,
            "UPDATE importacoes SET status=?, total_reg=?, total_erros=?, avisos=?"
            " WHERE id=?",
            ("ok", total_ok, total_erros, avisos_str, imp_id))
